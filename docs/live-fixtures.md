...
- Frontend: polls the published snapshot every 3 minutes and ignores it once `expiresAt` has passed
...
The five-minute Lambda schedule is intentionally conservative for the initial release. The three-minute frontend poll reduces the worst-case visibility delay without making one-minute CloudFront polling the default. With API-Football Pro's 7,500-request daily quota, the Lambda schedule can later be changed to one minute without changing the data model.
